...
public class BrokenUIL extends LinearOpMode {
    ...
    public double coneOffset = 89;
    public double webcamOffset = 155;
    public double safeClear = 440; //440
    //public double slidesUp = 1440;
    public double slidesUp = 1105;
...
//                    robot.intakeOuttake.horizontal.backwardRight.setPosition((BACKWARD_RIGHT_IN + BACKWARD_RIGHT_OUT) / 2);
                ...
                case SCORE:
                    ...
                    robot.butterfly.runToPosition(pose2.getX(), pose2.getY(), pose2.getHeading(),
                            .10, .25, -poseEstimate.getY(), poseEstimate.getX(),
                            poseEstimate.getHeading(), true);
                    ...
                    switch (score) {
                        case EXTEND:
                            ...
                            if (myTimer.time() < 750) {
                                if (cycles == -1) {
                                    robot.intakeOuttake.horizontal.backwardLeft.setPosition(linearProfile(750, myTimer.time(), 750, BACKWARD_LEFT_IN, (BACKWARD_LEFT_OUT - .084)));
                                    robot.intakeOuttake.horizontal.backwardRight.setPosition(linearProfile(750, myTimer.time(), 750, BACKWARD_RIGHT_IN, BACKWARD_RIGHT_OUT + .084));
                                } else if (cycles < 6) {
                                    robot.intakeOuttake.horizontal.backwardLeft.setPosition(linearProfile(750, myTimer.time(), 750, ((BACKWARD_LEFT_IN + BACKWARD_LEFT_OUT) / 2), (BACKWARD_LEFT_OUT - .084)));
                                    robot.intakeOuttake.horizontal.backwardRight.setPosition(linearProfile(750, myTimer.time(), 750, ((BACKWARD_RIGHT_IN + BACKWARD_RIGHT_OUT) / 2), BACKWARD_RIGHT_OUT + .084));
                                }
                            } else {
                                robot.intakeOuttake.horizontal.backwardLeft.setPosition(BACKWARD_LEFT_OUT - .084);
                                robot.intakeOuttake.horizontal.backwardRight.setPosition(BACKWARD_RIGHT_OUT + .084);
                            }
